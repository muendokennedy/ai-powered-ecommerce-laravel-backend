        ...
        $products = Product::with('images')
                ->where(function ($query) use ($search) {
                    $query->where('name', 'like', "%$search%")
                        ...
                        ->orWhere('product_sku_id', 'like', "%$search%");
                })
                ->latest()
                ->get();

                return response()->json(['products' => $products, 'authenticatedAdmin' => Auth::guard('admin')->user()]);
        
        } 
        $products = Product::with('images')->latest()->limit(4)->get();
        $productsAll = Product::with('images')->latest()->get();
        $stockOverview = $productsAll
